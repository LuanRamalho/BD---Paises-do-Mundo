...
function saveToLocalStorage() {
    ...
    localStorage.setItem('country_world', countriesData);
}

// Função para carregar os dados do localStorage
function loadFromLocalStorage() {
    const countriesData = localStorage.getItem('country_world');
    return countriesData ? JSON.parse(countriesData) : null;
}

// Função para buscar o país e exibir a bandeira
function searchCountry() {
    ...
    const countriesData = loadFromLocalStorage();
    
    if (countriesData) {
        for (let continent in countriesData) {
            for (let country in countriesData[continent]) {
                if (country.toLowerCase().includes(input)) {
                    const flagUrl = countriesData[continent][country];
                    const img = document.createElement('img');
                    img.src = flagUrl;
                    img.alt = `Bandeira de ${country}`;
...
// Salvar dados ao carregar a página
